...
if __name__ == "__main__":
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        timing = True  # Turn off to hide timing statistics
        ...
        test_schedule = [
            ...
            [2, 6, 6.0, [2.0, 0.0, 0.0]],
            [3, 6, 9.0, [3.0, 0.0, 0.0]],
            [4, 6, 10.0, [4.0, 0.0, 0.0]],
            [5, 6, 11.0, [0.0, 0.0, 0.0]],
        ]

        transfer_schedule(s, test_schedule, "test schedule", timing=timing)

        print("Printing schedule (filled)")
        print_schedule(s, max_entries=8)


        sleep(1)


        print("Playing schedule")

        activate_play_mode(s)
        play_start(s)
        sleep(8)
        step_current, steps, time_current = get_current_step_time(s)
        print(f"Current step: {step_current}/{steps} ({time_current} s)")
        sleep(8)
        play_stop(s)
        deactivate_play_mode(s)


        sleep(1)


        print("Setting apply_Bc_period from 0.1 to 0.001")
        set_apply_Bc_period(s, 0.001)


        sleep(1)


        print("Replaying schedule")
        activate_play_mode(s)
        play_start(s)
        sleep(8)
        step_current, steps, time_current = get_current_step_time(s)
